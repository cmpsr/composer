...
const getDropdownSingleSelectClasses = (disabled = false, invalid = false, itemSelected) => ({
  ...
  selectClasses: `appearance-none px-3 h-12 w-full bg-fill-form-enabled border rounded text-left ${invalid ? '!border-fill-system-error' : 'focus:border-outline-form-focus'} focus:outline-none text-form`,
  selectTextClasses: `${itemSelected ? 'text-dark-100' : 'text-outline-form-filled'}`,
  selectDisabledClasses: disabled ? 'bg-fill-form-disabled border-outline-form-disabled !text-outline-form-filled' : 'border-outline-form-filled hover:border-fill-state-primary-enabled',
