...
const getDropdownSingleSelectClasses = (disabled = false, invalid = false, itemSelected) => {
  return {
    ...
    selectTextClasses: `${itemSelected ? 'text-fill-forms-filled' : 'text-fill-forms-inactive'}`,
    selectFocusClasses: 'focus:border-outline-forms-focus focus:outline-none',
    selectDisabledClasses: disabled ? 'bg-fill-forms-disabled border-outline-forms-disabled !text-fill-forms-inactive' : 'border-outline-forms-filled hover:border-fill-primary-100',
