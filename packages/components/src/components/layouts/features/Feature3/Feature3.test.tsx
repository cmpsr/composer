...
describe('Feature1', () => {
  it('should match snapshot', () => {
    const hero1 = renderer.create(<Feature3 style={{}} title="title test" />);
    const tree = hero1.toJSON();
    expect(tree).toMatchSnapshot();
  });
  test('should render title', () => {
    const { getByText } = render(<Feature3 style={{}} title="title test" />);
    const element = getByText('title test');
    expect(element).toBeInTheDocument();
  });
  test('should render a Box component with class', () => {
    const { getByTestId } = render(
      <Feature3 style={{}} title="title test" className="foo" />,
    );
