...
import { Footer3, FOOTER3_DEFAULT_TEST_ID } from './Footer3';

describe('Footer3', () => {
  const testStyle = { background: { default: { bg: 'white' } } };
  it('should match snapshot', () => {
    const footer = renderer.create(<Footer3 style={testStyle} />);
    const tree = footer.toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('should render component', () => {
    const { getByTestId } = render(<Footer3 style={testStyle} />);
    const footer = getByTestId(FOOTER3_DEFAULT_TEST_ID);
  ...
  it('should render copy', () => {
    ...
    const { getByText } = render(<Footer3 style={testStyle} copy={testCopy} />);
  ...
  it('should render content logo', () => {
    ...
    const { getByTestId } = render(
      <Footer3 style={testStyle}>{renderedChildren}</Footer3>,
    );
