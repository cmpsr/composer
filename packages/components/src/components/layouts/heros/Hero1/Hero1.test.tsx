...
describe('Hero1', () => {
  ...
  it('Should render children', () => {
    const { getByText } = render(<Hero1 style={{}}>foo</Hero1>);
    const hero1 = getByText('foo');
    expect(hero1).toBeInTheDocument();
  });
  it('Should render with text', () => {
    const { getByText } = render(<Hero1 title="test title" style={{}} />);
    const hero1 = getByText('test title');
    expect(hero1).toBeInTheDocument();
  });
  it('Should render class', () => {
    const { getAllByTestId } = render(
      <Hero1 title="test title" style={{}} className="foo" />,
    );
    const hero1 = getAllByTestId(BOX_DEFAULT_TEST_ID);
    expect(hero1[0].classList).toContain('foo');
  });
  ...
  it('Should render subtitle', () => {
    ...
    const hero1 = getByText('test subtitle');
    expect(hero1).toBeInTheDocument();
  });
  it('Should render eyebrow', () => {
    ...
    const hero1 = getByText('eyebrow test');
    expect(hero1).toBeInTheDocument();
  });
});
