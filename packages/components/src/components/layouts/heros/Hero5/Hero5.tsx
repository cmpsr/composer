...
export const Hero5 = ({
  ...
  return (
    <Box
      size="screen"
      className={cn("flex justify-start", className, colorClasses)}
    >
      {mobileAsset && (
        <div className="md:hidden w-1/2 h-screen z-0 absolute">
          <Asset
            autoPlay
            asset={mobileAsset}
      ...
      {desktopAsset && !isMobile && (
        <Asset
          autoPlay
          asset={desktopAsset}
          className="hidden md:block w-1/2 h-full z-0 absolute"
          imageClassName="object-cover w-full h-full"
        />
      )}
      <Box size="full" className="flex justify-center items-center p-8">
        <Box size="content" className="flex justify-end items-center">
          <div className="relative z-10 max-w-hero-copy-small text-right md:text-left">
            {eyebrow && (
              <Typography
                tag="h4"
            ...
            {title && (
              <Typography
                tag="h2"
                type={TypographyTypes.Headline3}
                style={style.title}
              >
                {title}
              </Typography>
