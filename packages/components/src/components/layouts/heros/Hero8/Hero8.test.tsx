...
describe('Hero8', () => {
  ...
  it('should match snapshot', () => {
    ...
    const tree = hero8.toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('Should render children', () => {
    const { getByText } = render(<Hero8 style={{}}>foo</Hero8>);
    const hero8 = getByText('foo');
    expect(hero8).toBeInTheDocument();
  });
  it('Should render with text', () => {
    const { getByText } = render(<Hero8 title="test title" style={{}} />);
    const hero8 = getByText('test title');
    expect(hero8).toBeInTheDocument();
  });
  it('Should render class', () => {
    const { getAllByTestId } = render(
      <Hero8 title="test title" style={{}} className="foo" />,
    );
    const hero8 = getAllByTestId(BOX_DEFAULT_TEST_ID);
    expect(hero8[0].classList).toContain('foo');
  });
  ...
  it('Should render subtitle', () => {
    ...
    const hero8 = getByText('test subtitle');
    expect(hero8).toBeInTheDocument();
  });
  it('Should render eyebrow', () => {
    ...
    const hero8 = getByText('eyebrow test');
    expect(hero8).toBeInTheDocument();
  });
});
