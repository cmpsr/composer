...
import { Navigation1, NAVIGATION1_DEFAULT_TEST_ID } from '.';
import { BOX_DEFAULT_TEST_ID } from 'components/containers';

describe('Navigation1', () => {
  const testStyle = { background: { default: { bg: 'white' } } };
  it('should match snapshot', () => {
    const navigation = renderer.create(<Navigation1 style={testStyle} />);
    const tree = navigation.toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('should render component', () => {
    const { getByTestId } = render(<Navigation1 style={testStyle} />);
    const navigation = getByTestId(NAVIGATION1_DEFAULT_TEST_ID);
  ...
  it('should render content logo', () => {
    ...
    const { getByTestId } = render(
      <Navigation1 style={testStyle}>{renderedChildren}</Navigation1>,
    );
