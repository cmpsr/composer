...
import { Navigation2, NAVIGATION2_DEFAULT_TEST_ID } from '.';
import { BOX_DEFAULT_TEST_ID } from 'components/containers';

describe('Navigation2', () => {
  const testStyle = { background: { default: { bg: 'white' } } };
  it('should match snapshot', () => {
    const navigation = renderer.create(<Navigation2 style={testStyle} />);
    const tree = navigation.toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('should render component', () => {
    const { getByTestId } = render(<Navigation2 style={testStyle} />);
    const navigation = getByTestId(NAVIGATION2_DEFAULT_TEST_ID);
  ...
  it('should render content logo', () => {
    ...
    const { getByTestId } = render(
      <Navigation2 style={testStyle}>{renderedChildren}</Navigation2>,
    );
