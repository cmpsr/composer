...
import { Navigation8, NAVIGATION8_DEFAULT_TEST_ID } from '.';
import { BOX_DEFAULT_TEST_ID } from 'components/containers';

describe('Navigation8', () => {
  const testStyle = { background: { default: { bg: 'white' } } };
  it('should match snapshot', () => {
    const navigation = renderer.create(<Navigation8 style={testStyle} />);
    const tree = navigation.toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('should render component', () => {
    const { getByTestId } = render(<Navigation8 style={testStyle} />);
    const navigation = getByTestId(NAVIGATION8_DEFAULT_TEST_ID);
  ...
  it('should render content logo', () => {
    ...
    const { getByTestId } = render(
      <Navigation8 style={testStyle}>{renderedChildren}</Navigation8>,
    );
