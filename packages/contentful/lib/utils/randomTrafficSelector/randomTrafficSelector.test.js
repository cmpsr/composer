...
describe('randomTrafficSelector', () => {
  ...
  const tester = (randomValue, expected, items = traffics) => {
    const transformer = jest.fn().mockImplementation(i => i.value);
    const randomSpy = jest.spyOn(global.Math, 'random').mockReturnValue(randomValue);
    const selected = (0, _.randomTrafficSelector)(items, transformer);
