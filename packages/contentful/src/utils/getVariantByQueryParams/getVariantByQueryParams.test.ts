import { getVariantByQueryParams } from '.';

describe('getVariantByQueryParams', () => {
  const ref = 'ref';
  const variants = [
    { id: 1, name: '1', traffic: 1 },
    { id: 2, name: '2', match: [{ utm_option1: 'camp1' }], traffic: 0 },
    { id: 3, name: '3', match: [{ utm_option2: 'camp2' }], traffic: 0 },
  ];
  it('should return second items for camp1 param', () => {
    const value = getVariantByQueryParams(ref, variants, {
      utm_option1: 'camp1',
    });
  ...
  it('should return first item if multiple matches', () => {
    ...
    duplicatedMatchesVariants.push({
      id: 4,
      name: '4',
      match: [{ utm_option2: 'camp2' }],
      traffic: 0,
    });

    const value = getVariantByQueryParams(ref, duplicatedMatchesVariants, {
      utm_option2: 'camp2',
    });
    expect(value).toBeDefined();
    expect(value.id).toBe(variants[2].id);
