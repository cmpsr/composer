...
const mockExchangePublicToken = jest.fn().mockReturnValue({
...
const mockGetTransactions = jest.fn().mockReturnValue({
  accounts: [],
  transactions: []
});
const mockOwner = {
...
const mockGetBalance = jest.fn().mockReturnValue({
  accounts: []
});
...
const mockGetIncome = jest.fn().mockReturnValue({
  item: {},
  income: {}
});
...
const mockGetAssetReport = jest.fn().mockReturnValue({
  report: {
    asset_report_id: 'asset_report_id'
  }
});
...
describe('Plaid', () => {
  ...
  describe('getAccessToken', () => {
    test('should exchange public token', async () => {
      const plaid = new _.Plaid(config);
  ...
  describe('getAuth', () => {
    ...
    test('should retrieve auth for given accounts', async () => {
      ...
      expect(mockGetAuth).toBeCalledTimes(1);
      expect(mockGetAuth).toBeCalledWith(accessToken, {
        account_ids
      });
      expect(auth).toStrictEqual(mockGetAuth());
    });
  ...
  describe('getBalance', () => {
    ...
    test('should retrieve balance for given accounts', async () => {
      ...
      expect(mockGetBalance).toBeCalledTimes(1);
      expect(mockGetBalance).toBeCalledWith(accessToken, {
        account_ids
      });
      expect(balance).toStrictEqual(mockGetBalance());
    });
  ...
  const givenPublicTokenExchanged = async plaid => {
