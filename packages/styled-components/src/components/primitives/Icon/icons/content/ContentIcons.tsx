...
export const add_content = ({ type }: Props) => {
  const path = {
...
export const content_copy = ({ type }: Props) => {
  ...
  return (
    <>
      <path
        d={type === Types.Rounded ? 'M0 0h24v24H0z' : 'M0 0h24v24H0V0z'}
        fill="none"
      />
      <path d={type === Types.Rounded ? path[type] : path.filled} />
    </>
  );
};
...
export const weekend = ({ type }: Props) => {
  ...
  return (
    <path
      d={path[type]}
      transform={type === Types.Filled ? 'scale(0.5, 0.5)' : null}
    />
  );
};
