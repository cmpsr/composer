...
const conversation = {
  ...
  links: {
    participants: 'https://conversations.twilio.com/v1/Conversations/sid/Participants',
...
describe('TwilioConversation', () => {
  ...
  test('should initialize client', () => {
    new _TwilioConversation.default(config);
  ...
  describe('methods calling conversations as an object', () => {
    ...
    describe('getConversations', () => {
      test('should returns list of conversations', async () => {
        const twilioConversation = new _TwilioConversation.default(config);
    ...
    describe('createConversation', () => {
      test('should create conversation', async () => {
        ...
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedConversation = await twilioConversation.create(conversationName);
  ...
  describe('methods calling conversations as a function', () => {
    ...
    describe('getConversation', () => {
      test('should return conversation', async () => {
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedConversation = await twilioConversation.getConversation('sid');
    ...
    describe('Add participant', () => {
      ...
      test('should add participant with phone number', async () => {
        ...
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedParticipant = await twilioConversation.addSmsParticipant(conversationSid, invitedPhone, inviterPhone);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
      ...
      test('should add participant with identity', async () => {
        ...
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedParticipant = await twilioConversation.addParticipant(conversationSid, identity);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
        expect(mockCreateParticipant).toBeCalledTimes(1);
        expect(mockCreateParticipant).toBeCalledWith({
          identity
    ...
    describe('sendMessage', () => {
      test('should send message with author', async () => {
        ...
        const text = 'message';
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedMessage = await twilioConversation.sendMessage(conversationSid, authorSid, text);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
      ...
      test('should send message attributes', async () => {
        ...
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedMessage = await twilioConversation.sendMessage(conversationSid, authorSid, text, attributes);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
        expect(mockCreateMessage).toBeCalledTimes(1);
        expect(mockCreateMessage).toBeCalledWith({
          author: authorSid,
    ...
    describe('getMessages', () => {
      test('should fetch messages from conversation', async () => {
        const conversationSid = 'conversationSid';
        const twilioConversation = new _TwilioConversation.default(config);
        const returnedMessages = await twilioConversation.getMessages(conversationSid);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
    ...
    describe('addConversationWebhook', () => {
      ...
      test('should create webhook in twilio', async () => {
        const twilioConversation = new _TwilioConversation.default(config);
        const url = 'url';
        const returnedWebhook = await twilioConversation.addConversationWebhook(conversationSid, url);
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
      ...
      test('should use given method', async () => {
        const twilioConversation = new _TwilioConversation.default(config);
        const url = 'url';
        const returnedWebhook = await twilioConversation.addConversationWebhook(conversationSid, url, 'GET');
        expect(twilio().conversations.conversations).toBeCalledWith(conversationSid);
        expect(mockCreateWebhook).toBeCalledTimes(1);
        expect(mockCreateWebhook).toBeCalledWith({
          configuration: {
  ...
  describe('getMedia', () => {
    ...
    test('should return undefined if request fails', async () => {
      mockFetch.mockReturnValueOnce(_objectSpread(_objectSpread({}, mockFetch()), {}, {
        ok: false
      }));
      const twilioConversation = new _TwilioConversation.default(config);
    ...
    test('should return undefined if media content request fails', async () => {
      ...
      mockFetch.mockReturnValueOnce(_objectSpread({}, fetchResult)).mockReturnValueOnce(_objectSpread(_objectSpread({}, fetchResult), {}, {
        ok: false
      }));
      const twilioConversation = new _TwilioConversation.default(config);
      const media = await twilioConversation.getMedia(mediaId);
